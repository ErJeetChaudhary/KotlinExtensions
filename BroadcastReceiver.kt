...
/**
 * Created by Jitendra on 09:34, 18-05-2020
 */
class BroadcastReceiver<T>(context: T, constructor: Builder.() -> Unit) : LifecycleObserver where T : Context, T : LifecycleOwner {
...
sealed class Instructions {
    ...
    data class OnDataScheme(val scheme: String, val execution: Execution) : Instructions() {
        ...
        override fun matches(intent: Intent) = intent.data?.scheme == scheme
    ...
    data class OnCategory(val category: String, val execution: Execution) : Instructions() {
        ...
        override fun matches(intent: Intent) = intent.hasCategory(category)
